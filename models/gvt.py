...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # self.proj(x) -> [B, embed_dim, H // patch_size[0], W // patch_size[1]]
        # flatten(2) -> [B, embed_dim, H // patch_size[0] * W // patch_size[1]]
        # transpose(1, 2) -> [B, H // patch_size[0] * W // patch_size[1], embed_dim]
        # (means B, # patches, embed_dim)
    ...
    def forward_features(self, x):
        ...
        for i in range(len(self.depths)):
            ...
            if self.extra_norm:
                x = self.norm_list[i](x)
            x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
            #print(f"Layer {i} dim {x.shape}")
            #np.save(f"peg_feats/{tmp_identifier}/layer_{i}/out.npy", x.detach().cpu().numpy())
