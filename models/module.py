...
class FPNDecoder(nn.Module):
    def __init__(self, feat_chs):
        ...
        # if os.path.exists("fpn_output_feats"):
        #     shutil.rmtree("fpn_output_feats")
    ...
    def forward(self, conv01, conv11, conv21, conv31):
        ...
        if self.featuremap_count % 6 == 0 and self.featuremap_limit > (self.featuremap_count // 6):
            t0 = time.time_ns()
            os.makedirs(f"fpn_output_feats/{t0}")
            for i in range(4):
                np.save(f"fpn_output_feats/{t0}/out{i}.npy", outs[i].detach().cpu().numpy())
...
class TwinDecoderStage4(nn.Module):
    def __init__(self, args):
        ...
        self.smooth2 = nn.Sequential(nn.Conv2d(ch // 2, chs[-1], kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(chs[-1]), nn.ReLU(True))  # 128->64

        self.inner3 = nn.Conv2d(vit_chs[-4], chs[-1], kernel_size=1, stride=1, padding=0)
        self.smooth3 = nn.Sequential(nn.Conv2d(chs[-1], chs[-1], kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(chs[-1]), Swish())  # 64->64
        
        # if os.path.exists("decoder_feats"):
        #     shutil.rmtree("decoder_feats")
        # os.makedirs("decoder_feats", exist_ok=True)
    ...
    def forward(self, x1, x2, x3, x4):  # in:[1/8 ~ 1/64] out:[1/2,1/4,1/8]
        ...
        # import os
        #os.makedirs(f"decoder_feats/{t0}")

        #import numpy as np
        x = self.smooth1(self.upsampler0(x4) + self.inner1(x3))  # 1/64->1/32
        #np.save(f"decoder_feats/{t0}/x1.npy", x.detach().cpu().numpy())

        x = self.smooth2(F.upsample(x, scale_factor=2, mode='bilinear', align_corners=False) + self.inner2(x2))  # 1/32->1/16
        #np.save(f"decoder_feats/{t0}/x2.npy", x.detach().cpu().numpy())

        x = self.smooth3(F.upsample(x, scale_factor=2, mode='bilinear', align_corners=False) + self.inner3(x1))  # 1/16->1/8
        #np.save(f"decoder_feats/{t0}/x3.npy", x.detach().cpu().numpy())


        return x
    
class BilinearDecoderStage4(nn.Module):
    def __init__(self, args):
        super(BilinearDecoderStage4, self).__init__()
        ch, vit_chs = args['out_ch'], args['vit_ch']
        chs = args["out_chs"]
        ch = ch * 4  # 256
        self.upsampler1 = nn.Sequential(nn.Conv2d(vit_chs[-1], ch, kernel_size=1),nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                                       nn.BatchNorm2d(ch), nn.GELU())  # 256
        self.inner1 = nn.Conv2d(vit_chs[-2], ch, kernel_size=1, stride=1, padding=0)
        self.smooth1 = nn.Sequential(nn.Conv2d(ch, ch // 2, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(ch // 2), nn.ReLU(True))  # 256->128
        ...
        # if os.path.exists("decoder_feats_bilinear"):
        #     shutil.rmtree("decoder_feats_bilinear")
    ...
    def forward(self, x1, x2, x3, x4):  # in:[1/8 ~ 1/64] out:[1/2,1/4,1/8]
        ...
        #     t0 = time.time_ns()
        #     os.makedirs(f"decoder_feats_bilinear/{t0}", exist_ok=True)

        #np.save(f"decoder_feats_bilinear/{t0}/x0.npy", x4.detach().cpu().numpy())

        x = self.smooth1(self.upsampler1(x4) + self.inner1(x3))  # 1/64->1/32
        # if self.featuremap_limit > self.featuremap_count:
        #     np.save(f"decoder_feats_bilinear/{t0}/x1.npy", x.detach().cpu().numpy())

        x = self.smooth2(F.upsample(x, scale_factor=2, mode='bilinear', align_corners=False) + self.inner2(x2))  # 1/32->1/16
        # if self.featuremap_limit > self.featuremap_count:
        #     np.save(f"decoder_feats_bilinear/{t0}/x2.npy", x.detach().cpu().numpy())

        x = self.smooth3(F.upsample(x, scale_factor=2, mode='bilinear', align_corners=False) + self.inner3(x1))  # 1/16->1/8
        # if self.featuremap_limit > self.featuremap_count:
        #     np.save(f"decoder_feats_bilinear/{t0}/x3.npy", x.detach().cpu().numpy())
        #     self.featuremap_count += 1
...
class TwinDecoderStage4V3(nn.Module):
    def __init__(self, args, use_resnet=False):
        super(TwinDecoderStage4V3, self).__init__()
        ch, vit_chs = args['out_ch'], args['vit_ch']
        ...
        resnet_ch = [64, 128, 256, 2048]
        in_chs = vit_chs if not use_resnet else resnet_ch

        self.first_upsampler3 = nn.Sequential(nn.Conv2d(in_chs[-1], in_chs[-1], kernel_size=1),nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                                       nn.BatchNorm2d(in_chs[-1]), nn.GELU())
        self.first_upsampler2 = nn.Sequential(nn.Conv2d(in_chs[-2], in_chs[-2], kernel_size=1),nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                                       nn.BatchNorm2d(in_chs[-2]), nn.GELU())
        self.first_upsampler1 = nn.Sequential(nn.Conv2d(in_chs[-3], in_chs[-3], kernel_size=1),nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                                       nn.BatchNorm2d(in_chs[-3]), nn.GELU())
        self.first_upsampler0 = nn.Sequential(nn.Conv2d(in_chs[-4], in_chs[-4], kernel_size=1),nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                                       nn.BatchNorm2d(in_chs[-4]), nn.GELU())

        # self.upsampler0 = nn.Sequential(nn.ConvTranspose2d(in_chs[-1], ch, 4, stride=2, padding=1),
        #                                 nn.BatchNorm2d(ch), nn.GELU())  # 256
        self.upsampler0 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(in_chs[-1], ch, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(ch), nn.GELU())  # 256
        self.inner1 = nn.Conv2d(in_chs[-2], ch, kernel_size=1, stride=1, padding=0)
        self.smooth1 = nn.Sequential(nn.Conv2d(ch, ch // 2, kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(ch // 2), nn.GELU())  # 256->128

        self.inner2 = nn.Conv2d(in_chs[-3], ch // 2, kernel_size=1, stride=1, padding=0)
        self.smooth2 = nn.Sequential(nn.Conv2d(ch // 2, out_chs[-1], kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(out_chs[-1]), nn.GELU())  # 128->64

        self.inner3 = nn.Conv2d(in_chs[-4], out_chs[-1], kernel_size=1, stride=1, padding=0)
        self.smooth3 = nn.Sequential(nn.Conv2d(out_chs[-1], out_chs[-2], kernel_size=3, stride=1, padding=1),
                                     nn.BatchNorm2d(out_chs[-2]), nn.GELU())  # 64->32

        # self.decoder1 = nn.Sequential(nn.ConvTranspose2d(out_chs[-2], out_chs[-3], 4, stride=2, padding=1),
        #                               nn.BatchNorm2d(out_chs[-3]), nn.GELU()) # 32->16
        # self.decoder2 = nn.Sequential(nn.ConvTranspose2d(out_chs[-3], out_chs[-4], 4, stride=2, padding=1),
        #                               nn.BatchNorm2d(out_chs[-4]), nn.GELU())
        self.decoder1 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(out_chs[-2], out_chs[-3], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(out_chs[-3]), nn.GELU()) # 32->16
        self.decoder2 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(out_chs[-3], out_chs[-4], kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(out_chs[-4]), nn.GELU())
        self.featuremap_count = 0
        self.featuremap_limit = 20
        # remove directory recursively
    ...
    def forward(self, x1, x2, x3, x4):  # in:[1/8 ~ 1/64] out:[1/2,1/4,1/8]
        x1, x2, x3, x4 = self.first_upsampler0(x1), self.first_upsampler1(x2), self.first_upsampler2(x3), self.first_upsampler3(x4)
        x = self.smooth1(self.upsampler0(x4) + self.inner1(x3))  # 1/64->1/32
        x = self.smooth2(F.upsample(x, scale_factor=2, mode='bilinear', align_corners=False) + self.inner2(x2))  # 1/32->1/16
        out1 = self.smooth3(F.upsample(x, scale_factor=2, mode='bilinear', align_corners=False) + self.inner3(x1))  # 1/16->1/8
        out2 = self.decoder1(out1)
        ...
        if self.featuremap_limit > self.featuremap_count:
            t0 = time.time_ns()
            os.makedirs(f"decoder_feats_bilinear/{t0}", exist_ok=True)
            np.save(f"decoder_feats_bilinear/{t0}/x0.npy", x.detach().cpu().numpy())
            np.save(f"decoder_feats_bilinear/{t0}/x1.npy", out1.detach().cpu().numpy())
            np.save(f"decoder_feats_bilinear/{t0}/x2.npy", out2.detach().cpu().numpy())
            np.save(f"decoder_feats_bilinear/{t0}/x3.npy", out3.detach().cpu().numpy())
            self.featuremap_count += 1
