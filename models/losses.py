...
def ce_loss_stage4(inputs, depth_gt_ms, mask_ms, dlossw, focal=False, gamma=0.0, inverse_depth=True, refine_steps=4):
    depth_loss_weights = dlossw

    loss_dict = {}
    for (stage_inputs, stage_key) in [(inputs[k], k) for k in ['stage1', 'stage2', 'stage3', 'stage4'][:refine_steps]]:
        depth_gt = depth_gt_ms[stage_key]
        depth_values = stage_inputs["depth_values"]  # (b, d, h, w)
        prob_volume_pre = stage_inputs["prob_volume_pre"].to(torch.float32)
        mask = mask_ms[stage_key]
