...
class BaseTrainer:
    ...
    def __init__(self, model, optimizer, config, writer=None, rank=0, ddp=False, epoch_inter=1):
        ...
        self.iter_save_period = cfg_trainer['iter_save_period']
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.rank == 0 and epoch % self.epoch_inter == 0:
                ...
                if epoch % self.save_period == 0 and self.rank == 0:
                    if best is True:
                        self._save_checkpoint(epoch, 0, save_best=True)
                    self._save_checkpoint(epoch, 0, save_last=True)
    ...
    def _save_checkpoint(self, epoch, iteration, save_best=False, save_last=False):
        """
        Saving checkpoints
        :param epoch: current epoch number
        ...
        if save_best:
            ...
            last_path = str(self.checkpoint_dir / f'model_last_{epoch}.pth')
