...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, optimizer, config, data_loader, valid_data_loader=None, lr_scheduler=None, writer=None, rank=0, ddp=False,
        ...
        self.is_dtu = config['data_loader'][0].get('type', "TOTELoader") == "DTULoader"

        self.refine_steps = config["arch"]["args"].get("refine_steps", 4)
        ...
        # Create folder repro_test if not exists
        if self.rank == 0:
            if not os.path.exists("repro_test"):
                os.makedirs("repro_test")
    ...
    def _train_epoch(self, epoch):
        ...
        for dl in self.data_loader:
            for batch_idx, sample in enumerate(dl):
                ...
                iters_to_accumulate = max(1, imgs.shape[0] // bs)
                total_loss = 0.0
                ...
                for bi in range(iters_to_accumulate):
                    ...
                    for k in cam_params:
                        ...
                        if ref_bg_masks is not None:
                            ref_bg_masks_tmp[k] = ref_bg_masks[k][b_start:b_end]
                    ...
                    if self.config["trainer"].get("mask_bg", False):
                        for i, key in enumerate(["stage1", "stage2", "stage3", "stage4"][:self.refine_steps]):
                            mask = ref_bg_masks_tmp[key]
                            outputs[key]["depth"][~mask] = 0.0
                    ...
                    if self.depth_type == 're':
                        ...
                        loss_dict = ce_loss_stage4(outputs, depth_gt_ms_tmp, mask_ms_tmp, dlossw=[1, 1, 1, 1],
                                                   focal=self.focal, gamma=self.gamma, inverse_depth=self.inverse_depth, refine_steps=self.refine_steps)
                    ...
                    # gradient_sum = torch.tensor(0.0).cuda()
                    # for name, param in self.model.named_parameters():
                    #     if param.requires_grad and param.grad is not None:
                    #         gradient_sum += param.grad.abs().sum()
                    # if torch.isnan(gradient_sum):
                    #     print('Rank', str(self.rank) + ':', 'NaN in gradient of step', global_step, '!!!')
                    # with open(exp_repro_fname, 'a') as f:
                    #     f.write(str(gradient_sum.item()) + '\n')
                ...
                if batch_idx % self.log_step == 0 and self.rank == 0:
                    ...
                    wandb.log(
                        {"epoch" : epoch, "train_step": (epoch-1) * len(dl) + batch_idx, "iter": batch_idx, "total_loss": total_loss.item(), "lr": self.optimizer.param_groups[0]["lr"], "scale": int(self.scaler.get_scale())}
                    )
                ...
                if batch_idx > 0 and batch_idx % self.iter_save_period == 0 and self.rank == 0:
                    self._save_checkpoint(epoch, batch_idx, save_best=True)
    ...
    def _valid_epoch(self, epoch):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, dl in enumerate(self.valid_data_loader):
                if self.is_dtu and i % 10 != 0:
                    continue
                ...
                for batch_idx, sample in enumerate(tqdm(dl)):
                    ...
                    mask_ms = sample_cuda["mask"]
                    num_stage = self.refine_steps
                    ref_bg_mask = sample_cuda.get("ref_background_masks", {}).get(f"stage{num_stage}", None)
                    ...
                    #scalar_outputs["test_step"] = epoch * len(self.valid_data_loader) + batch_idx
                    scalar_cpy = scalar_outputs.copy()
                    scalar_cpy["test_step"] = (epoch-1) * len(self.valid_data_loader) + batch_idx
                    wandb.log(scalar_cpy)
